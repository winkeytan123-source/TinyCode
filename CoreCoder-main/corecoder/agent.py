...
class Agent:
    ...
    def _tool_schemas(self) -> list[dict]:
        # t.schema() returns the tool's name, description, and JSON schema for arguments
    ...
    def chat(self, user_input: str, on_token=None, on_tool=None) -> str:
        ...
        for _ in range(self.max_rounds):
            ...
            if len(resp.tool_calls) == 1:
                ...
                # on_tool callback is for UI display of tool calls as they happen; it doesn't affect execution
                if on_tool:
    ...
    def _exec_tools_parallel(self, tool_calls, on_tool=None) -> list[str]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as pool:
            # 执行 self._exec_tool(tc) 这个函数
            # 非阻塞：submit 提交任务后会立刻返回，不会等待 _exec_tool 真正执行完毕。
