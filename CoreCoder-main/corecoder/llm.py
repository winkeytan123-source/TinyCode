    ...
    def chat(
        ...
        for chunk in stream:
            ...
            if delta.content:
                ...
                # 在cli.py中，on_token作为函数会将增量内容实时打印到控制台
                if on_token:
...
class LiteLLM(LLM):
    ...
    def chat(
        ...
        for chunk in stream:
            # 用 getattr(obj, "attr", default) 比直接 obj.attr 更安全，不会因为属性不存在而抛出 AttributeError
    ...
    def _call_with_retry(self, params: dict, max_retries: int = 3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 字符串模糊匹配错误类型，litellm可能会抛出不同的异常，无法直接捕获特定异常类
                err = str(e).lower()
                is_transient = any(
                    kw in err
